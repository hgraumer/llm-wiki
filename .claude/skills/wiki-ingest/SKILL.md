...
title: <source title>
aliases:
  - <common short name, if any>
tags:
  - source
  - <domain tag, e.g. mathematics, ai, history>
sources:
  - <slug>
updated: <today>
---
...
<2-3 paragraph synthesis — your own words, not abstract copy-paste. Use $...$ for
any inline math and $$...$$ for standalone equations.>
...
> [!note] Central result
> <The single most important claim or finding from this source.>
...
### 5b. Math and formatting — apply throughout

This wiki renders in Obsidian. Apply these rules in every section you write:

- **Math:** every formula, symbol, or equation uses LaTeX — `$...$` inline, `$$...$$` for display. Never write math as plain text (e.g. write `$f(x) = x^2$`, not `f(x) = x^2`).
- **Multi-line derivations:** use `$$\begin{align}...\end{align}$$`.
- **Callouts:** use sparingly for genuine structural value:
  - `> [!note]` — single most important insight in a section
  - `> [!warning]` — known limitation, caveat, or common misunderstanding
  - `> [!example]` — concrete worked example
  - `> [!summary]-` (collapsible) — long proof sketch or derivation detail

### 5c. Cite as you write — do not skip
...
### 5d. Self-check before continuing
...
aliases:
  - <common alternative name, e.g. "Hinton" for "Geoffrey Hinton" — omit if none>
tags:
  - entity   (or concept)
  - <domain tag>
sources:
...
<synthesis across all sources that discuss this. Use $...$ for inline math,
$$...$$ for standalone definitions, theorems, or key equations.>
